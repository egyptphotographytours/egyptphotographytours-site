#!/usr/bin/env python3
"""
Smart Translation Engine v4.0
=============================
✅ DeepL API: Official API, never blocks, best quality
✅ Safe Chunking: Max 40 strings per chunk
✅ DOM Firewall: Prevents <html><body> injection bugs
✅ DOM Cleanup: Deletes broken nested tags from old runs
✅ Content Hashing: Only translates when actual text changes
✅ Force Translate: Manually trigger a specific file
✅ --seo-only mode: Repairs hreflang, menus & DOM without translating
✅ Sitemap: NOT included — handled by separate workflow
"""
...
# ==========================================
# TRANSLATION ENGINE (DeepL First)
# ==========================================
class TranslationEngine:
    ...
    def _deepl(self, text, target):
        from deep_translator import DeepLTranslator
        lang = 'zh' if target == 'zh-CN' else target
        return DeepLTranslator(api_key=self.deepl_key, source='en', target=lang).translate(text)
    ...
    def translate(self, text, target):
        ...
        engines.extend(['bing', 'google', 'mymemory'])

        if self.consecutive_failures >= ENGINE_SWITCH_THRESHOLD:
            engines = engines[1:] + engines[:1]

        last_err = None
        for eng in engines:
            try:
                if eng == 'deepl':
                    result = self._run_with_timeout(self._deepl, text, target)
                elif eng == 'google':
...
def inject_global_seo():
    ...
    for base, langs in page_map.items():
        for lang_code, rel_path in langs.items():
            ...
            head.append(soup.new_tag('link', rel='canonical', href=get_absolute_url(lang_code, base)))
            for sib_lang in langs:
                hl = 'zh' if sib_lang == 'zh-CN' else sib_lang
                head.append(soup.new_tag('link', rel='alternate', hreflang=hl, href=get_absolute_url(sib_lang, base)))
            head.append(soup.new_tag('link', rel='alternate', hreflang='x-default', href=get_absolute_url("en", base)))
...
# ==========================================
def main():
    parser = argparse.ArgumentParser(description='Smart Translation Engine')
    ...
    print("=" * 60, flush=True)
    print("🚀 Smart Translation Engine v4.0", flush=True)
    print("=" * 60, flush=True)
