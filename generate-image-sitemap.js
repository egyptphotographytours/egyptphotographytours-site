...
// ===============================
// Convert HTML path → clean canonical URL
// ===============================
...
function getMeta(filename) {
  ...
  return {
    title: `${formatted} | Egypt Photography Tours`,
    caption:
...
function getAllImages() {
  return globSync('images/**/*.{jpg,jpeg,png,webp,gif}', {
    ...
    .filter(img =>
      !EXCLUDED.some(ex =>
        img.filename.toLowerCase().includes(ex)
      )
    );
}
...
// ===============================
function generate() {
  const imageMap = scanHTMLForImages();
  ...
  images.forEach(({ path: imgPath, filename }) => {
    ...
    pages.forEach(page => {
      ...
      xml += `    <image:image>\n`;
      xml += `      <image:loc>${DOMAIN}/${imgPath}</image:loc>\n`;
