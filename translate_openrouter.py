...
- Pauses 60s if it hits OpenRouter's rate limit
"""
...
# ----------------------------------------------------------------------
def translate_batch_with_openrouter(texts, target_lang_code):
    """Sends a batch of text to OpenRouter and returns translated text."""
    if not texts: return []
    
    lang_name = LANG_NAMES.get(target_lang_code, target_lang_code)
    
    prompt = f"""You are an expert luxury travel copywriter. 
Translate the following JSON array of strings into {lang_name}. 

CRITICAL RULES:
1. Tone: Persuasive, welcoming, professional, and luxurious.
2. Do NOT translate URLs, emails, phone numbers, or brand names.
3. Return ONLY a valid JSON array of the translated strings. No markdown, no explanations.
...
"""
    ...
    # ✅ 5 RETRIES WITH 60 SECOND PAUSE FOR RATE LIMITS
    for attempt in range(5): 
        try:
            response = requests.post(OR_URL, headers=OR_HEADERS, json=payload, timeout=60)
            ...
            if response.status_code == 200:
                ...
                print(f"    ⏳ Hit OpenRouter rate limit. Pausing for 60 seconds...")
                time.sleep(60)
            else:
...
def git_commit_and_push(target_lang, file_paths, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            subprocess.run(["git", "commit", "-m", f"translate-openrouter({target_lang}): batch ({len(file_paths)} files)"], check=False, capture_output=True)
